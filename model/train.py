...
from model.transformer import DecoderOnlyTransformer, EncoderDecoderTransfomer


@torch.no_grad()
def evaluate_gen(model, val_loader, loss_fn, mask, config, device, max_batches=50):
    model.eval()
    total_loss = 0.0
    n_batches = 0
...
def train_gen():
    ...
    for epoch in range(last_epoch, config.num_epochs):
        ...
        val_loss = evaluate_gen(model, val_loader, loss_fn, mask, config, device)
...
def train_translation():
    tokenize_and_pad("train")
    tokenize_and_pad("validation")
    train_ds = TranslationDataset("train")
    train_loader = DataLoader(train_ds, batch_size=64, shuffle=True, num_workers=0)
    val_ds = TranslationDataset("validation")
    val_loader = DataLoader(val_ds, batch_size=64, shuffle=False, num_workers=0)
    ...
    model = EncoderDecoderTransfomer(
